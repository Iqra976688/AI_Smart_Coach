#!/usr/bin/env python3
# app.py — AI Smart Coach (Streamlit)
# Full, self-contained Streamlit app integrating pose_logic, rag_chain, and agentic_ai.
# Copy this file exactly and run: streamlit run app.py
...
# streamlit-webrtc import (required for live webcam in-stream)
# If not installed, the app will show a friendly error and degrade gracefully.
try:
    from streamlit_webrtc import VideoProcessorBase, WebRtcMode, webrtc_streamer
except Exception as e:
    VideoProcessorBase = object  # fallback type (won't be used)
    WebRtcMode = None

    def webrtc_streamer(*args, **kwargs):  # type: ignore
        raise RuntimeError(
            "streamlit-webrtc is required for live webcam. Install it with `pip install streamlit-webrtc`."
        )


# Project modules (these must exist in your repo)
# pose_logic should expose: CoachReport, ExerciseCounter, ExerciseProfile, FOCUS_AREAS,
# calculate_angle, generate_coaching_report, get_profile, list_categories, list_profiles_by_category
...
# rag_chain must expose CoachBrain and CoachVoice
from rag_chain import CoachBrain, CoachVoice

# agentic AI module (safe fallback)
try:
    from agentic_ai import AgenticCoach
except Exception:
    # Provide a no-op AgenticCoach if module missing so imports never fail
    class AgenticCoach:
        def __init__(self, *a, **k):
            pass

        def enhance_feedback(self, cue, movement, metrics=None, user_id=None):
            return ""
...
FEEDBACK_DEBOUNCE_SEC = 5.0  # seconds between sending the same headline
# Minor margin for choosing similar triplets (not used directly but kept for compatibility)
TRIPLET_SWITCH_MARGIN = 6.0

# ---- Services ----
coach_brain = CoachBrain()    # RAG / LLM feedback (falls back to static text if LLM not available)
coach_voice = CoachVoice()    # TTS wrapper (gTTS with pyttsx3 fallback)
agentic_coach = AgenticCoach()  # Agentic decision-maker (safe fallback above)


# ---- Streamlit rerun helper (compatible across streamlit versions) ----
...
# ---- Helpers to extract landmarks safely ----
...
# ---- Video processor: MediaPipe Pose + overlay + metrics ----
class PoseProcessor(VideoProcessorBase):
    ...
    def __init__(self, feedback_queue: queue.Queue[object], profile_key: str, focus_area: str):
        ...
        self._pose = mp.solutions.pose.Pose(
            min_detection_confidence=0.5, min_tracking_confidence=0.6, model_complexity=1
        )
...
def main() -> None:
    ...
    with stats_col:
        ...
        st.metric("Joint Angle", f"{metrics.get('angle', '—'):.1f}" if isinstance(metrics.get('angle', 0), (int, float)) else "—")
        rom_value = metrics.get("rom_last") or metrics.get("rom_current")
        st.metric("Range of Motion", f"{rom_value:.1f}" if isinstance(rom_value, (int, float)) else "—")

        st.metric("Tempo (last)", f"{metrics.get('tempo_last'):.2f}s" if isinstance(metrics.get('tempo_last'), (int, float)) else "—")

        if isinstance(report, CoachReport):
            st.metric("Balance", f"{getattr(report, 'balance_score', '—')}")
            st.metric("Speed", f"{getattr(report, 'speed_score', '—')}")
